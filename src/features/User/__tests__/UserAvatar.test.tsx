...
describe('UserAvatar', () => {
  describe('enable Auth', () => {
    ...
    it('should show zfChat and default avatar when the user is not logged in and enable auth', () => {
      act(() => {
        useUserStore.setState({ enableAuth: () => true, isSignedIn: false, user: undefined });
      });

      render(<UserAvatar />);
      expect(screen.getByAltText('zfChat')).toBeInTheDocument();
      expect(screen.getByAltText('zfChat')).toHaveAttribute('src', DEFAULT_USER_AVATAR_URL);
    });
  });

  describe('disable Auth', () => {
    it('should show zfChat and default avatar when the user is not logged in and disabled auth', () => {
      enableAuth = false;
      act(() => {
